...
defmodule TimelessLogsDashboard.HistoricalSource.Local do
  ...
  @impl true
  def subscribe(_opts), do: TimelessLogs.subscribe()

  @impl true
  def unsubscribe(_opts), do: TimelessLogs.unsubscribe()
end
...
defmodule TimelessLogsDashboard.HistoricalSource.DataPlane do
  ...
  defp normalize_stats(stats) when is_map(stats) do
    %{
      ...
      zstd_bytes: value(stats, :compressed_bytes, 0),
      openzl_blocks: 0,
      openzl_bytes: 0,
      compression_raw_bytes_in: value(stats, :optimize_raw_input_bytes, 0),
      compression_compressed_bytes_out: value(stats, :optimize_raw_output_bytes, 0),
